...
{ /*  primosbag.c  */
    ...
    while (tam * num_procs >= n)
    {
        tam -= 50000;
    }
    /* Registra o tempo inicial de execução do programa */
    t_inicial = MPI_Wtime();
    /* Envia pedaços com tam números para cada processo */
    if (meu_ranque == 0)
    {
        for (dest = 1, inicio = 3; dest < num_procs && inicio < n; dest++, inicio += tam)
        {
        ...
        while (stop < (num_procs - 1))
        {
            ...
            /* Envia um nvo pedaço com tam números para o mesmo processo*/
            MPI_Send(&inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);
            inicio += tam;
        }
    }
    else
    {
        ...
        while (estado.MPI_TAG != 99)
        {
            ...
            {
                for (i = inicio, cont = 0; i < (inicio + tam) && i < n; i += 2)
                    if (primo(i) == 1)
                        cont++;
